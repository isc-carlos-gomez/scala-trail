...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    (w.toLowerCase groupBy ((e: Char) => e) map {
      case (char, freq) => (char, freq.length)
    } toList).sorted
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] =
    dictionary map {
      (word: Word) => wordOccurrences(word) -> word
    } groupBy {
      case (occurrences, _) => occurrences
    } map {
      case (occurrences, groups) => occurrences -> groups.map {
        case (_, word) => word
      }
    }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    x map {
      case (char, freq) => char -> (freq - yMap(char))
    } filter (_._2 > 0)
  }
